...
const WoofyHoverImage: React.FC<WoofyHoverImageProps> = ({
  ...
  const hoverTextureRef = useRef<THREE.Texture | null>(null);
  ...
  const initializeEffect = useCallback(() => {
    ...
    loader.load(src, (texture) => {
      ...
      const uniforms = {
        ...
        u_hoverTexture: { value: hoverTexture || texture },
        u_hasHoverTexture: { value: !!hoverTexture },
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (hoverTextureRef.current) {
        hoverTextureRef.current.dispose();
        hoverTextureRef.current = null;
      }
