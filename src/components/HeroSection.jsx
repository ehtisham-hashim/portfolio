...
const Iridescence = dynamic(() => import("./backgrounds/Iridescence"), {
  ssr: false,
});

const SPRING_CONFIG = { damping: 25, stiffness: 120 };
const IRIDESCENCE_COLOR = [1, 1, 1];
...
export default function HeroSection() {
  ...
  const tX3 = useTransform(
    smoothMouseX,
    [-1, 1],
    [-maxOffset * 0.8, maxOffset * 0.8],
  );
  const tY3 = useTransform(
    smoothMouseY,
    [-1, 1],
    [-maxOffset * 0.8, maxOffset * 0.8],
  );
  ...
  return (
    <section
      ref={ref}
      className="main-section relative w-full min-h-screen bg-white flex items-start justify-center pt-[12vh] pb-12 overflow-hidden"
    >
      <div className="relative w-[90vw] h-[80vh] md:h-[100vh] rounded-[2rem] md:rounded-[3.5rem] overflow-hidden shadow-2xl border border-slate-200 bg-white flex items-center justify-center">
        ...
        >
          <Image
            ...
            placeholder="blur"
            priority
            className={`object-cover transition-opacity duration-700 ease-in-out ${step === 0 ? "opacity-100" : "opacity-0"}`}
          />
          <Image
            src={mongoImg}
            alt="Mongo"
            ...
            placeholder="blur"
            priority
            className={`object-cover transition-opacity duration-700 ease-in-out ${step === 1 ? "opacity-100" : "opacity-0"}`}
          />
          <Image
            src={postgresImg}
            alt="Postgres"
            ...
            placeholder="blur"
            priority
            className={`object-cover transition-opacity duration-700 ease-in-out ${step === 2 ? "opacity-100" : "opacity-0"}`}
          />
        </motion.div>
        ...
        >
          ...
          <Image
            ...
            placeholder="blur"
            priority
            className={`object-cover transition-opacity duration-700 ease-in-out ${step === 1 || step === 2 ? "opacity-100" : "opacity-0"}`}
          />
        </motion.div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-30 pointer-events-none px-4">
          <h1
            className="text-4xl sm:text-5xl md:text-5xl lg:text-6xl font-bold text-center text-slate-800 max-w-5xl leading-tight transition-colors duration-500"
          ...
          >
            ...
            <span
              className={
                step === 0
                  ? "text-[#0066FF] transition-colors duration-500"
